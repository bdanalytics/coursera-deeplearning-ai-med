...
def to_one_hot(dataframe, columns):
    ...
    for clm in columns:
        one_hot_columns = [df_clm for df_clm in one_hot_df.columns if clm + '_' in df_clm]
        #print(f"debug: clm:{clm}; one_hot_columns:{one_hot_columns}")
        one_hot_df = one_hot_df.drop(columns = one_hot_columns[0])
...
def harrell_c(y_true, scores, event):
    ...
    #print(f"debug: permissible:{permissible}; concordant:{concordant}; ties:{ties}")
    result = ((1.0 * concordant) + (0.5 * ties)) / permissible
