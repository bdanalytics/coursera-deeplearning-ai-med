...
def frac_censored(df):
    ...
    result = np.sum(df['Event'] == 0) * 1.0 / df.shape[0]
...
def naive_estimator(t, df):
    ...
    X_t = np.sum(df['Time'] > t)
    M_t = np.sum((df['Event'] == 1) | (df['Time'] > t))
    S_t = X_t * 1.0 / M_t
...
def HomemadeKM(df):
    ...
    #print(f"debug: observed_event_times:{observed_event_times}")
    # iterate through event times
    for t in observed_event_times:
  
        # compute n_t, number of people who survive to time t
        n_t = np.sum(df['Time'] >= t)
              #np.sum(df['Time'] > t)
  
        # compute d_t, number of people who die at time t
        d_t = np.sum((df['Time'] == t) & (df['Event'] == 1))
              #np.sum((df['Event'] == 1) | (df['Time'] > t))
        
        # update p
        p_t = (1 - (d_t * 1.0 / n_t))
        p *= p_t
        #print(f"debug: t:{t}\n d_t:{d_t}; n_t:{n_t}; p_t:{p_t}\n p:{p}")
